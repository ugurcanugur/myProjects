...
# load("~/Project/data/Mexico_Violence/18_concentrated/Z_outputs/mex_historical.RData")

load(here("data","mex_historic.RData"))

# prehisp_poly = mex_historical$lugo_paper$lugo_prehisp_poly
# 
# prehisp_points = mex_historical$lugo_paper$lugo_prehisp_points
# 
# colonial_points = mex_historical$digging_early_mex_repo$primary_sources$polygon_NO$DPYT_Suma.shp

 # save(list = c("prehisp_poly","prehisp_points","colonial_points"), file = 
 #        here("data","mex_historic.RData"))
...
-   This returns the the mean altitude for each grain size class. \tiny  - it is also possible to get a raster with calculated statistics for each zone by setting the `as.raster` argument to `TRUE`.
